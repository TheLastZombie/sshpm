module.exports = (cli, data, flags) => {
  ...
  if (!fs.existsSync(outRem)) {
    throw Error('Remmina configuration directory does not exist')
  }

  if (!flags.keep) {
    fs.readdirSync(outRem).forEach((file) => {
      const conf = fs.readFileSync(path.resolve(outRem, file), 'utf-8')
      if (conf.match(/\[remmina\]/) && conf.match(/sshpm=/)) {
        fs.unlinkSync(path.resolve(outRem, file))
      }
    })
  }

  data.forEach((element) => {
    let tempRem = fs.readFileSync(
      path.resolve(__dirname, '..', 'assets', '1.remmina'),
      'utf-8'
    )
    tempRem = tempRem
