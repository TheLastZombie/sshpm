module.exports = (cli, data, flags) => {
  ...
  if (flags.init) {
    if (!fs.existsSync(path.dirname(outElec))) {
      fs.mkdirSync(path.dirname(outElec))
    }
    fs.openSync(outElec, 'a')
  }
  if (!fs.existsSync(outElec)) {
    throw Error('electerm configuration file does not exist')
  }
  ...
  data.forEach((element) => {
    let tempElec = fs.readFileSync(
      path.resolve(__dirname, '..', 'assets', 'electerm.bookmarks.nedb'),
      'utf-8'
    )
    let conf = fs.readFileSync(outElec, 'utf-8')
    tempElec = tempElec
