module.exports = async (cli, data, flags) => {
  ...
  if (process.platform !== 'win32') {
    throw Error(
      'KiTTY uses registry entries, which are not supported on non-Windows systems'
    )
  }

  if (
    !flags.init &&
    !(await Registry.has('HKCU\\SOFTWARE\\9bis.com\\KiTTY\\Sessions'))
  ) {
    throw Error('KiTTY configuration key does not exist')
  }
    ...
    if (conf) {
      for (const element of Object.keys(conf)) {
        const x = await Registry.get(
          'HKCU\\SOFTWARE\\9bis.com\\KiTTY\\Sessions\\' + element
        )
