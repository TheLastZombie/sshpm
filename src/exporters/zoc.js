module.exports = (cli, data, flags) => {
  ...
  if (flags.init) {
    if (!fs.existsSync(path.dirname(outZoc))) {
      fs.mkdirSync(path.dirname(outZoc))
    }
    fs.writeFileSync(outZoc, 'ZOC\n[DATA]\n[/DATA]')
  }
  if (!fs.existsSync(outZoc)) {
    throw Error('ZOC configuration file does not exist')
  }
  if (fs.readFileSync(outZoc, 'utf-8').includes('\nCrypto=')) {
    throw Error(
      'ZOC configuration file is encrypted and, as such, not supported'
    )
  }
  ...
  data.forEach((element) => {
    let tempZoc = fs.readFileSync(
      path.resolve(__dirname, '..', 'assets', 'HostDirectory.zhd'),
      'utf-8'
    )
    let conf = iconv.decode(fs.readFileSync(outZoc), 'iso-8859-1')
    tempZoc = tempZoc
