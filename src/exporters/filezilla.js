module.exports = (cli, data, flags) => {
  ...
  if (flags.init) {
    ...
    fs.writeFileSync(outFza, '<?xml version="1.0"?>\n<FileZilla3>\n<Servers>\n</Servers>\n</FileZilla3>')
  }
  if (!fs.existsSync(outFza)) throw Error('FileZilla configuration file does not exist')
  ...
  data.forEach(element => {
    let tempFza = fs.readFileSync(path.resolve(__dirname, '..', 'assets', 'sitemanager.xml'), 'utf-8')
    let conf = fs.readFileSync(outFza, 'utf-8')
    tempFza = tempFza
      ...
      .replace(/\$\(PASS\)/g, Buffer.from(element.pass).toString('base64') || '')
