module.exports = (cli, data, flags) => {
  ...
  if (flags.init) {
    if (!fs.existsSync(path.dirname(outRcl))) {
      fs.mkdirSync(path.dirname(outRcl))
    }
    fs.openSync(outRcl, 'a')
  }
  if (!fs.existsSync(outRcl)) {
    throw Error('rclone configuration file does not exist')
  }
  if (
    fs
      .readFileSync(outRcl, 'utf-8')
      .startsWith('# Encrypted rclone configuration File')
  ) {
    throw Error(
      'rclone configuration file is encrypted and, as such, not supported'
    )
  }
  ...
  data.forEach((element) => {
    ...
    if (element.pass) {
      pass = childProcess
        .spawnSync(flags.exec || pathRClone, ['obscure', element.pass], {
          encoding: 'utf-8'
        })
        .output[1].trim()
    }
    let tempRcl = fs.readFileSync(
      path.resolve(__dirname, '..', 'assets', 'rclone.conf'),
      'utf-8'
    )
    let conf = fs.readFileSync(outRcl, 'utf-8')
    tempRcl = tempRcl
