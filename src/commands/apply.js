...
class ApplyCommand extends Command {
  async run () {
    ...
    require(path.resolve(__dirname, '..', 'exporters', args.program + '.js'))(
      this,
      data,
      flags
    )
...
const path = require('path')
const exporters = fs
  .readdirSync(path.resolve(__dirname, '..', 'exporters'))
...
ApplyCommand.args = [
  {
    name: 'program',
    description: 'program to send profiles to',
    required: true,
    options: exporters
  }
]
...
ApplyCommand.flags = {
  ...
  init: flags.boolean({
    char: 'i',
    description: 'create non-existent configuration files'
  }),
  keep: flags.boolean({
    char: 'k',
    description: 'keep previous profiles sent by sshpm'
  }),
  conf: flags.string({
    char: 'c',
  ...
  exec: flags.string({
    char: 'x',
    description: 'path to executable required by exporter'
  })
}

ApplyCommand.examples = fs
  .readdirSync(path.resolve(__dirname, '..', 'exporters'))
  .map((x) => '$ sshpm apply ' + path.parse(x).name)
