...
class LoginCommand extends Command {
  async run () {
    ...
    if (data.map((x) => x.name).includes(name)) {
      throw Error('a profile with the same name already exists')
    }
    if (flags.key && !fs.existsSync(flags.key) && !flags.force) {
      throw Error('key file does not exist, use -f to continue')
    }
    ...
    fs.writeFileSync(
      file,
      JSON.stringify(data.sort((x, y) => x.name.localeCompare(y.name)))
    )
  }
}
...
LoginCommand.args = [{ name: 'name', description: 'profile name' }]
